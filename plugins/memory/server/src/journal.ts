...
export function appendJournal(op: string, data: Record<string, unknown>): void {
  try {
    appendFileSync(
      join(getDataDir(), JOURNAL_FILE),
      JSON.stringify({ ts: new Date().toISOString(), op, ...data }) + "\n"
    );
  } catch (err) {
...
export function writeSnapshot(): number {
  const db = getDb();
  const rows = db
...
export function ensureJournalBootstrap(): void {
  ...
  try {
    ...
    const lines = rows.map((row) =>
      JSON.stringify({ ts, op: "snapshot", memory: rowToMemory(row) })
    );
...
export function importFromJsonl(): { imported: number; skipped: number } {
  const path = join(getDataDir(), SNAPSHOT_FILE);
