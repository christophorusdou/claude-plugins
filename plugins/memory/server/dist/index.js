...
// --- memory_store ---
server.tool("memory_store", "Store a knowledge entry in the cross-project archive. For patterns, gotchas, debug insights, decisions, or facts that benefit from semantic search. NOT for user preferences or session feedback (use built-in memory for those). Auto-detects project scope. Deduplicates within same scope.", {
...
// --- memory_recall ---
server.tool("memory_recall", "Search the knowledge archive using semantic + keyword hybrid search. Use as a fallback when built-in memory doesn't have relevant context, or for cross-project search. If results contradict built-in MEMORY.md, built-in is authoritative. Auto-detects project from cwd for two-pass search (project + global).", {
...
// --- memory_manage ---
server.tool("memory_manage", "Manage the knowledge archive. Actions: update, delete, upvote, downvote, list, stats, cleanup, audit, consolidate, age, merge, sync, import.", {
    action: z
        .enum([
        "update",
        "delete",
    ...
    switch (args.action) {
        ...
        case "cleanup": {
            ...
            if (candidates.length === 0)
                return text("No cleanup candidates found. All entries are healthy!");
        ...
        case "sync": {
            if (!args.operation)
                return text("Error: operation is required for sync (push/pull/export/rebuild)");
