...
function cmdSessionStart() {
    ...
    if (!existsSync(join(getDataDir(), "memory.db"))) {
        emitSessionStart("Memory plugin active. No memories stored yet — use memory_store to save the first one.");
        return;
    }
    const db = getDb();
    ...
    if (sync?.status === "failed") {
        parts.push(`[memory] Last background git push FAILED at ${sync.ts}. Run memory_manage action:"sync" operation:"push" to retry and see the error.`);
    }
    ...
    if (count >= CURATE_MIN_ENTRIES && daysSinceCuration() >= CURATE_NUDGE_DAYS) {
        ...
        if (groups.length > 0) {
            ...
            parts.push(`[memory] ${count} entries, ${when}, and near-duplicate groups exist. Run /mem curate to consolidate.`);
        }
    }
...
function cmdVerify() {
    ...
    const fts = db.prepare("SELECT COUNT(*) AS c FROM memories_fts").get().c;
    ...
    for (const q of goldenQueries) {
        ...
        for (const r of results) {
            console.log(`  [${r.memory.id.slice(0, 8)}] rel:${r.relevance.toFixed(2)} final:${r.final_score.toFixed(3)} ${r.memory.content.slice(0, 90).replace(/\n/g, " ")}`);
        }
    }
