...
export function ftsSearch(db, match, opts = {}) {
    const conditions = ["memories_fts MATCH ?", "m.lifecycle_state != 'merged'"];
    const params = [match];
    ...
    try {
        rows = db
            .prepare(`SELECT m.*, -bm25(memories_fts, 4.0, 2.0, 1.0) AS bm25_rank
         FROM memories_fts
