 ...
 */
export async function importMemoryMd(filePath, project) {
    const content = readFileSync(filePath, "utf-8");
    const fileName = basename(filePath);
    // Detect project from path if not provided
    const inferredProject = project ?? inferProjectFromPath(filePath);
    const entries = [];
    // Split by ## headers
    ...
    for (const entry of entries) {
        ...
        result.entries.push({
            content: entry.content.length > 80
        ...
        if (storeResult.status === "created")
            ...
            result.duplicates++;
        else if (storeResult.status === "near-duplicate")
